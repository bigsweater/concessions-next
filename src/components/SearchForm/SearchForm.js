...
export default function SearchForm({ initialGenres }) {
  const { searchParams, updateQueryString } = React.useContext(QueryStringContext)
          ...
          <div className="col-span-full">
            <div className="flex gap-6">
              ...
              {searchParams.search && (
                <button
                  ...
                  onClick={() => updateQueryString({search: ''})}
                >
            ...
            <div className="mt-2">
              <input
                ...
                onChange={e => updateQueryString({search: e.target.value || ''})}
          ...
          <div className="flex gap-6">
            ...
            {searchParams.genre && (
              <button
                type="button"
                className="inline-flex items-center px-1 py-1 text-sm font-semibold text-indigo-600 hover:text-indigo-900 focus:ring-1 ring-inset ring-gray-300"
                onClick={() => updateQueryString({genre: ''})}
              >
                <XMarkIcon aria-hidden="true" className="h-5 w-5 text-indigo-300 hover:text-indigo-900" />{" "}
                Clear
              </button>
                  ...
                  <li className="relative flex gap-x-3" key={genre.id}>
                    <div className="flex h-6 items-center">
                      <input
                        ...
                        checked={searchParams.genre === genre.title}
                        value={genre.title}
                        onChange={(e) => updateQueryString({genre: e.target.value || ''})}
                      />
                    </div>
